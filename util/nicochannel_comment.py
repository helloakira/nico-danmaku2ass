...
if __name__=='__main__' :
    ...
    parser.add_argument('--fc_site_id','-f', help='fc_site_id in audience_token')
    args = parser.parse_args()
    video_url = args.video_url
    #fc_site_id = args.fc_site_id

    # Check Video URL
    if re.compile('video/[0-9A-Za-z]+$').search(args.video_url) :
...
parse = urlparse(args.video_url)
headers = {'User-Agent': ua, 'Accept': 'application/json, text/plain, */*', 'Accept-Encoding': 'gzip, deflate, br', 'fc_use_device': 'null',
           'Origin': f'https://{parse.netloc}', 'DNT': '1', 'Connection': 'keep-alive', 'Referer': f'https://{parse.netloc}', 
           'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'same-site', 'TE': 'trailers',
           'accept': 'application/json, text/plain, */*', 'accept-language': 'ja,en-US;q=0.9,en;q=0.8'}

# Get fc_site_id
ch_fcid_request = requests.get(f'https://api.{parse.netloc}/fc/content_providers/channels', headers=headers)
for data in ch_fcid_request.json().get("data", {}).get("content_providers", []):
    if data["domain"] == f'{args.video_url}'.split('/video')[0]:
        fc_site_id = data.get("id")
try:
    fc_site_id = str(fc_site_id)
except:
    ch_fcid_request = requests.get(f'https://{parse.netloc}/site/settings.json', headers=headers)
    fc_site_id = str(ch_fcid_request.json().get('fanclub_site_id'))

# Get Video Info
headers = {'User-Agent': ua, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'ja', 'Accept-Encoding': 'gzip, deflate, br', 
           'fc_site_id': '105', 'fc_use_device': 'null', 'Origin': 'https://nicochannel.jp', 'DNT': '1', 'Connection': 'keep-alive', 
           'Referer': 'https://nicochannel.jp/', 'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'same-site', 
           'TE': 'trailers', 'accept': 'application/json, text/plain, */*', 'accept-language': 'ja,en-US;q=0.9,en;q=0.8',
